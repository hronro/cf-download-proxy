import {fastDecodeURIComponent as fastDecode} from './fast-decode-uri-component'
import {Node, types as NODE_TYPES} from './node'
import {HTTP_METHODS} from './constants'
import {acceptVersionStrategy} from './accept-version'
...
export class Router {
  constructor(opts) {
    ...
    if (opts.defaultRoute) {
      if (typeof opts.defaultRoute !== 'function') {
        throw new Error('The default route must be a function')
      }
    ...
    if (opts.onBadUrl) {
      if (typeof opts.onBadUrl !== 'function') {
        throw new Error('The bad url handler must be a function')
      }
    ...
    this.caseSensitive = opts.caseSensitive == null ? true : opts.caseSensitive
  ...
  _insert(method, path, kind, params, handler, store, regex, version) {
    const route = path
    ...
    while (true) {
      ...
      if (len < prefixLen) {
        ...
        if (len === pathLen) {
          if (version) {
            if (currentNode.getVersionHandler(version, method)) {
              throw new Error(`Method '${method}' already declared for route '${route}' version '${version}'`)
            }
  ...
  lookup(req) {
    var handle = this.find(req.method, sanitizeUrl(req.url), this.versioning.deriveVersion(req))
    if (handle === null)
      return this._defaultRoute(req)
    return handle.handler(req, handle.params, handle.store)
  }
  ...
  _getWildcardNode(node, method, path, len) {
    if (node === null)
      return null
  ...
  all(path, handler, store) {
    this.on(HTTP_METHODS, path, handler, store)
  }
}

for (const m of HTTP_METHODS) {
  const methodName = m.toLowerCase()
  if (Router.prototype[m]) throw new Error('Method already exists: ' + m)

export class Handlers {
  constructor(handlers) {
    ...
    this.ACL = h['ACL'] ?? null
    this.BIND = h['BIND'] ?? null
    this.CHECKOUT = h['CHECKOUT'] ?? null
    this.CONNECT = h['CONNECT'] ?? null
    this.COPY = h['COPY'] ?? null
    this.DELETE = h['DELETE'] ?? null
    this.GET = h['GET'] ?? null
    this.HEAD = h['HEAD'] ?? null
    this.LINK = h['LINK'] ?? null
    this.LOCK = h['LOCK'] ?? null
    this['M-SEARCH'] = h['M-SEARCH'] ?? null
    this.MERGE = h['MERGE'] ?? null
    this.MKACTIVITY = h['MKACTIVITY'] ?? null
    this.MKCALENDAR = h['MKCALENDAR'] ?? null
    this.MKCOL = h['MKCOL'] ?? null
    this.MOVE = h['MOVE'] ?? null
    this.NOTIFY = h['NOTIFY'] ?? null
    this.OPTIONS = h['OPTIONS'] ?? null
    this.PATCH = h['PATCH'] ?? null
    this.POST = h['POST'] ?? null
    this.PROPFIND = h['PROPFIND'] ?? null
    this.PROPPATCH = h['PROPPATCH'] ?? null
    this.PURGE = h['PURGE'] ?? null
    this.PUT = h['PUT'] ?? null
    this.REBIND = h['REBIND'] ?? null
    this.REPORT = h['REPORT'] ?? null
    this.SEARCH = h['SEARCH'] ?? null
    this.SOURCE = h['SOURCE'] ?? null
    this.SUBSCRIBE = h['SUBSCRIBE'] ?? null
    this.TRACE = h['TRACE'] ?? null
    this.UNBIND = h['UNBIND'] ?? null
    this.UNLINK = h['UNLINK'] ?? null
    this.UNLOCK = h['UNLOCK'] ?? null
    this.UNSUBSCRIBE = h['UNSUBSCRIBE'] ?? null 
  }
}
...
Node.prototype.setVersionHandler = function (version, method, handler, params, store) {
  ...
  if (handlers[method] != null) {
    throw new Error(`There is already an handler with version '${version}' and method '${method}'`)
  }
